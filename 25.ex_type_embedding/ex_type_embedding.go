...
func (c *CpuTemp) AveragCpuTemp() int {
	sum := 0
...
func (m *MemoryUsage) AveragMemoryUsage() int {
	sum := 0
...
func main() {
	...
	fmt.Printf("Average bandwidth usage: %v/n", dash.AverageBandwidth())
	fmt.Printf("Average temp: %v/n", dash.AveragCpuTemp())
	fmt.Printf("Average memory usage: %v/n", dash.AveragMemoryUsage())
}
